...
scenario_result = "results/base_scenario/"
...
EK = (gp.quicksum(C_f[f] * X_f[f, t] for f in manu for t in period) + gp.quicksum(C_g[g] * X_g[g, t] for g in remanu for t in period) + gp.quicksum(C_h[h] * X_h[h, t]  for h in hybrid for t in period) + gp.quicksum(C_j[a] * X_j[a, t]  for a in collect for t in period) + gp.quicksum(C_u[a] * X_u[a, t]  for a in disassembly for t in period) + gp.quicksum(C_v[a] * X_v[a, t]  for a in recycling for t in period) + gp.quicksum(C_w[a] * X_w[a, t]  for a in disposal for t in period) ) / T
...
log_filename = scenario_result + "mip_log.txt"
objective_filename = scenario_result + "objective_function.txt"
...
# 	comparison.at['Gesamtkosten', 'Basisszenario'] = round(total_cost)
# 	comparison.at['Einrichtungskosten', 'Basisszenario'] = round(EK.getValue())
# 	comparison.at['Produktionskosten', 'Basisszenario'] = round(PK.getValue())
# 	comparison.at['Transportkosten', 'Basisszenario'] = round(TK.getValue())
# 	comparison.at['Emissionskosten', 'Basisszenario'] = round(KK.getValue())
# 	comparison.at['CO2-Ausstoß', 'Basisszenario'] = round(CO2.X)
...
if m.Status == GRB.OPTIMAL:
	...
	m.computeIIS()
	m.write(scenario_result + "/model.ilp")
