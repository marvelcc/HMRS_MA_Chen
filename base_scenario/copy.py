...
# pi = 0.085 # carbon legal per kg 
# pi_p = 0.134 # carbon penalty per kg
pi = m.addVar(vtype=GRB.CONTINUOUS, lb=0.085, ub=0.085, name="pi")
pi_p = m.addVar(vtype=GRB.CONTINUOUS, lb=0.134, ub=0.134, name="pi_penalty")
...
# Add constraints
# (1)
constrCAP_f = m.addConstrs(((gp.quicksum(Q_fi[f, i, t] for i in warehouse) <= CAP_f[t, f] * X_f[f, t]) for f in manu for t in period), name='constrCAP_f')

# (2)
constrCAP_g = m.addConstrs((gp.quicksum(Q_gi[a, b, t] for b in warehouse) <= CAP_g[t, a] * X_g[a, t] for a in remanu for t in period), name='c2')

# (3)
constrCAP_h = m.addConstrs((gp.quicksum(Q_hi[a, b, t] for b in warehouse) <= CAP_h[t, a] * X_h[a, t] for a in hybrid for t in period), name='c3')

# (4)
constrCAP_h_bar = m.addConstrs((gp.quicksum(Q_hi_bar[a, b, t] for b in warehouse) <= CAP_h_bar[t, a] * X_h[a, t] for a in hybrid for t in period), name='c4')
